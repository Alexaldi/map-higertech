...
@section('title', $tutorial->title . ' | Tutorial Higertech')
@section('description', $tutorial->excerpt ?: \Illuminate\Support\Str::limit(strip_tags($tutorial->content), 160))
        ...
        <section class="relative isolate overflow-hidden bg-slate-50 pb-14 pt-12 dark:bg-[#0B1120] sm:pb-20 sm:pt-16">
            ...
            <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400" aria-label="Breadcrumb">
                    ...
                    <a href="{{ route('tutorials') }}" class="hover:text-blue-600 dark:hover:text-cyan-300">Tutorial</a><span>/</span>
                    <span class="text-slate-400 truncate max-w-xs">{{ $tutorial->title }}</span>
                </nav>
                <div class="mt-10 max-w-4xl">
                    ...
                    @endif
                    <h1 class="text-4xl font-black leading-tight tracking-tight text-slate-950 dark:text-white sm:text-5xl lg:text-6xl">{{ $tutorial->title }}</h1>
                    @if ($tutorial->excerpt)
                        <p class="mt-6 max-w-3xl text-base leading-8 text-slate-600 dark:text-slate-300 sm:text-lg">{{ $tutorial->excerpt }}</p>
                    @endif
        ...
        <section class="-mt-2 pb-20 sm:pb-28">
            <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                @if ($tutorial->video_url)
                    <div class="overflow-hidden rounded-3xl border border-slate-200 bg-slate-900 shadow-2xl shadow-blue-950/10 dark:border-slate-800">
                        <div class="relative w-full aspect-video">
                            @php
                                $videoUrl = $tutorial->video_url;
                                if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=|live/)|youtu\.be/)([^"&?/ ]{11})%i', $videoUrl, $match)) {
                                    $videoUrl = 'https://www.youtube.com/embed/' . $match[1];
                                }
                            @endphp
                            <iframe class="absolute top-0 left-0 w-full h-full" src="{{ $videoUrl }}" title="Video Tutorial" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
                        </div>
                    </div>
                @elseif ($tutorial->image_url)
                    <figure class="overflow-hidden rounded-3xl border border-slate-200 bg-slate-100 shadow-2xl shadow-blue-950/10 dark:border-slate-800 dark:bg-slate-900">
                        <img src="{{ $tutorial->image_url }}" alt="{{ $tutorial->title }}" class="aspect-[16/8] w-full object-cover" fetchpriority="high">
                        <figcaption class="px-5 py-3 text-xs text-slate-500 dark:text-slate-400">Dokumentasi: {{ $tutorial->title }}</figcaption>
                    </figure>
                @endif
                ...
                <div class="mt-12 grid gap-12 lg:grid-cols-[minmax(0,1fr)_260px]">
                    <article class="article-prose min-w-0 text-[17px] leading-8 text-slate-600 dark:text-slate-300">
                        <div class="mb-9">
                            <h2 class="article-description-title">Materi Tutorial</h2>
                    ...
                    {{-- Sidebar with recent tutorials --}}
                    <aside class="lg:pt-3">
                        <div class="sticky top-28 space-y-6">
                            @if (isset($recentTutorials) && $recentTutorials->isNotEmpty())
                                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-[#131D36]">
                                    ...
                                    <div class="mt-4 space-y-4">
                                        @foreach ($recentTutorials as $recent)
                                            <a href="{{ route('tutorials.show', $recent->slug) }}" class="group block">
                                                <span class="text-xs text-slate-400 font-mono">{{ $recent->duration_text }}</span>
                ...
                <div class="mt-14 flex flex-col items-start justify-between gap-5 border-t border-slate-200 pt-8 dark:border-slate-800 sm:flex-row sm:items-center">
                    <a href="{{ route('tutorials') }}" class="inline-flex items-center gap-2 text-sm font-bold text-blue-600 hover:text-cyan-600 dark:text-cyan-400">
                        <span aria-hidden="true">←</span> Kembali ke semua tutorial
                    </a>
                    <a href="https://wa.me/628112332182" target="_blank" rel="noopener noreferrer"
